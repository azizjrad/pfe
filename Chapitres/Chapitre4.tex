...
{\Large\textbf{Sprint : Gestion des processus des réservations}}
...
L'administrateur peut filtrer les données par période ou par agence. \\
\hline
\textbf{Scénario alternatif} &
Pas d'exceptions \\
\hline
...
\label{Diag_classes_participantes_internaute_s2}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Chapitres/Figures/classes_participantes__Internaute_s2.png}
    \caption{Diagramme des classes participantes par acteur Internaute}
    \label{fig:classes_participantes_internaute_s2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Chapitres/Figures/classes_participantes__administrateur_s2.png}
...
\label{sec:diag_seq_internaute_s2}
\subsubsection{Diagrammes de séquence détaillé du cas d’utilisation « Interagir avec Chatbot »}
\label{sec:diag_seq_det_chatbot}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Chapitres/figures/diag_seq_det/seq_det_s2/seq_det_chatbot.png}
...
\hline
id & BigInt & Identifiant unique de la réservation & PRIMARY KEY, AUTO\_INCREMENT, NOT NULL \\
\hline
user\_id & BigInt & Identifiant du client & FOREIGN KEY (users), NOT NULL \\
\hline
vehicle\_id & BigInt & Identifiant du véhicule & FOREIGN KEY (vehicles), NOT NULL \\
\hline
...
\hline
status & Enum('pending','confirmed','ongoing','completed') & Statut de la réservation & NOT NULL, DEFAULT 'pending' \\
\hline
actual\_return\_date & Date & Date de retour effective & NULLABLE \\
\hline
is\_late\_return & Tinyint(1) & Indique si le retour est en retard & NOT NULL, DEFAULT 0 \\
\hline
cancellation\_reason & Text & Raison d'annulation & NULLABLE \\
\hline
notes & Text & Notes internes & NULLABLE \\
\hline
created\_at & Timestamp & Date de création & NULLABLE \\
\hline
updated\_at & Timestamp & Date de modification & NULLABLE \\
\hline
client\_birth\_date & Date & Date de naissance du client & NULLABLE \\
\hline
driver\_first\_name & Varchar(255) & Prénom du conducteur & NULLABLE \\
\hline
driver\_last\_name & Varchar(255) & Nom du conducteur & NULLABLE \\
\hline
driver\_birth\_date & Date & Date de naissance du conducteur & NULLABLE \\
\hline
driver\_license\_number & Varchar(255) & Numéro de permis conducteur & NULLABLE \\
\hline
driver\_license\_date & Date & Date d'émission du permis & NULLABLE \\
\hline
\end{tabular}%
}
\end{table}
...
\textbf{Champs} & \textbf{Type} & \textbf{Libellé} & \textbf{Contraintes} \\
\hline
id & BigInt UNSIGNED & Identifiant unique & PRIMARY KEY, AUTO\_INCREMENT, NOT NULL \\
\hline
vehicle\_id & BigInt UNSIGNED & Identifiant du véhicule & FOREIGN KEY (vehicles), NOT NULL \\
\hline
price & Decimal(10,2) & Prix journalier appliqué & NOT NULL \\
\hline
start\_date & Date & Date de début de validité du tarif & NULLABLE \\
\hline
end\_date & Date & Date de fin de validité du tarif & NULLABLE \\
\hline
\end{tabular}%
}
\end{table}
